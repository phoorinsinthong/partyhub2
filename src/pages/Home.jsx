...
import { Plus, Users, Crown, UserPlus, QrCode, X, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
...
const Home = () => {
  ...
  const [avatarEmoji, setAvatarEmoji] = useState(savedAvatar.emoji || '🐱');
  const [avatarColor, setAvatarColor] = useState(savedAvatar.color || getAvatarColor('default'));
  ...
  // Auto-assign avatar on first visit
  useEffect(() => {
    if (!savedAvatar.emoji) {
      const emoji = getRandomAvatar();
      const color = getRandomColor();
      setAvatarEmoji(emoji);
      setAvatarColor(color);
      saveAvatar(emoji, color);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Only ~1 in 3 clients attempt cleanup to avoid redundant deletes
    ...
    const unsubscribe = onValue(ref(db, 'rooms'), (snapshot) => {
      ...
      if (shouldCleanup) {
        Object.entries(data).forEach(([code, room]) => {
          ...
          const allOffline = room.players && Object.values(room.players).every(p => p.online === false);
          ...
          const shouldDelete = playerCount === 0 || age > ROOM_MAX_AGE || playingTooLong || (allOffline && hostGone);
  ...
  const handleCreateRoom = async () => {
    ...
    if (!nickname.trim()) { setError('โปรดใส่ชื่อเล่นก่อนนะ!'); return; }
    if (!validateNickname(nickname.trim())) return;
    if (!rateLimitCreateRoom()) { setError('สร้างห้องบ่อยเกินไป รอสักครู่นะ'); return; }
    setIsSubmitting(true);
    try {
      await authReady;
      if (!auth.currentUser) { setError('ยังไม่ได้เข้าสู่ระบบ — ลองรีเฟรช'); return; }
      ...
      await set(ref(db, `rooms/${code}`), {
        host: nickname.trim(), status: 'waiting', createdAt: Date.now(), currentGame: null,
        players: { [nickname.trim()]: {
          name: nickname.trim(), isHost: true, joinedAt: Date.now(),
  ...
  const handleJoinRoom = async (roomCode) => {
    if (isSubmitting) return;
    if (!nickname.trim()) { setError('ใส่ชื่อเล่นก่อนเข้าห้องนะ!'); return; }
    if (!validateNickname(nickname.trim())) return;
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 min-h-0 overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
            {waitingRooms.map((room) => {
              // Show player avatars — use key as fallback name
              const playerEntries = room.players
                ? Object.entries(room.players).map(([key, val]) => ({ ...val, name: val.name || key }))
                : [];
              return (
            ...
            {playingRooms.map((room) => (
              <div key={room.code} className="card p-4 opacity-40 pointer-events-none">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-warm-50 flex-center shrink-0">
                    <span className="text-xl">🎮</span>
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-[14px] tracking-wide">{room.code}</span>
                      <span className="text-[9px] font-extrabold bg-warm-100 text-warm-500 px-2 py-0.5 rounded-full">เล่นอยู่</span>
                    </div>
                    <span className="text-[11px] text-olive-400">{room.host} • {room.playerCount} คน</span>
                  </div>
                </div>
              </div>
            ))}
