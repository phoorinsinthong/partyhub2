...
const Lobby = () => {
  ...
  usePresence(roomId, nickname, isHost);
  ...
  useEffect(() => {
    if (roomId && nickname) saveSession(roomId, nickname);
  }, [roomId, nickname]);
  ...
  useEffect(() => {
    const unsub = onValue(ref(db, `rooms/${roomId}`), (snap) => {
      ...
      const d = snap.val();
      if (d.players && !d.players[nickname]) { if (!leavingRef.current) { clearSession(); navigate('/', { state: { kicked: true } }); } return; }
      if (d.players?.[nickname]) { const h = d.players[nickname].isHost; setIsHost(h); localStorage.setItem('isHost', h?'true':'false'); }
      setRoomData(d);
      if (d.status === 'playing' || d.status === 'finished') navigate(`/game/${roomId}`);
    });
    return () => unsub();
  ...
  const handleStartGame = async () => {
    ...
    feedback('gameStart');
    try {
      await update(ref(db, `rooms/${roomId}`), { status: 'playing', currentGame: selectedGame, gameData: { status: 'waiting', startTime: Date.now() } });
    } finally {
  ...
  const handleLeaveRoom = async () => {
    ...
    try {
      ...
      else await remove(ref(db, `rooms/${roomId}/players/${nickname}`));
      navigate('/');
    } catch {
  ...
  const handleKickPlayer = async (name) => { if(isHost) await remove(ref(db, `rooms/${roomId}/players/${name}`)); };
  ...
  const players = Object.entries(roomData.players||{}).map(([key,val])=>({key,...val, name: val.name || key}));
  const myPlayer = roomData.players?.[nickname];
    ...
    <div className="animate-fade-in flex flex-col flex-1">
      ...
      <div className="flex-center gap-3 mb-4">
        <div className="inline-flex items-center gap-2 bg-sage-50 border-2 border-sage-100 rounded-full px-4 py-2">
          ...
          <span className="text-[12px] font-extrabold text-sage-700">รอผู้เล่น • {players.length} คน</span>
        </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      ...
                      {p.key === nickname && <span className="text-[9px] font-extrabold text-sage-600 bg-sage-100 px-1.5 py-0.5 rounded-md">คุณ</span>}
                    </div>
                  </div>
                </div>
                {isHost && !p.isHost && (
        ...
        <section className="flex flex-col gap-4 flex-1">
          ...
          <div className="mt-auto pt-2">
            {(() => {
              ...
              const notEnough = players.length < minP;
              return (
                <>
                  <button
